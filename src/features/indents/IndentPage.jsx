// IndentPage.jsx - copied and adapted from QuotationPage.jsx
    ...
    <div className="bg-white rounded-lg sm:rounded-2xl shadow-md sm:shadow-lg p-3 sm:p-4 border border-[#BCE0FD]">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-0">
        <div className="flex items-center">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-[#0B3861]">Indent Management</h2>
        </div>
      </div>
      <div className="flex gap-6 mb-6 justify-center">
        {(userRole === 'lab_assistant' || userRole === 'central_store_admin') && (
          ...
          >
            ...
            <span className="text-[#0B3861] font-bold text-base text-center">Create Indent</span>
          </div>
        )}
        <div
          className={`flex flex-col items-center justify-center w-48 h-48 rounded-2xl shadow-xl border-2 border-[#BCE0FD] cursor-pointer transition-all duration-200 select-none
            ${showIndentList ? 'bg-[#BCE0FD] scale-90' : 'bg-[#F5F9FD] hover:bg-[#BCE0FD]'}
          `}
          onClick={() => {
            if (showIndentList) setShowIndentList(false);
            else {
              setShowIndentList(true);
              setShowFormCard(false);
            }
          }}
        >
          <DocumentIcon className="mb-2 text-[#0B3861] w-8 h-8" />
          <span className="text-[#0B3861] font-bold text-base text-center">Indent List</span>
        </div>
      </div>
      {showFormCard && (
