...
const QuotationDetailEnhanced = ({ quotation, isOpen, onClose, onRefresh }) => {
  ...
  const [localComments, setLocalComments] = useState([]); // For real-time updates
  ...
  // Initialize local comments for real-time updates
  useEffect(() => {
  ...
  const handleSubmitMessage = async () => {
    ...
    try {
      await axios.post(
        ...
        { text: newMessage }, // Backend expects 'text', not 'comments'
  ...
  const handleSaveEdits = async () => {
    setIsLoading(true);
    try {
      // Prepare updates for all item types
      const updateData = {};
      
      // Chemical updates
      if (editedQuotation.chemicals) {
        updateData.chemicalUpdates = editedQuotation.chemicals.map((chem, index) => ({
          index,
      ...
      // Equipment updates
      if (editedQuotation.equipment) {
        updateData.equipmentUpdates = editedQuotation.equipment.map((eq, index) => ({
          index,
          remarks: eq.remarks || ''
        }));
      }
      ...
      // Glassware updates
      if (editedQuotation.glassware) {
        updateData.glasswareUpdates = editedQuotation.glassware.map((glass, index) => ({
          index,
      ...
      await axios.patch(
        `https://backend-pharmacy-5541.onrender.com/api/quotations/${quotation._id}/items`,
        updateData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(11);
    if (chemicalTotal > 0) {
      doc.text(`Chemical Total: Rs.${chemicalTotal.toFixed(2)}`, margin, yPos);
      yPos += 6;
    }
    if (equipmentTotal > 0) {
      doc.text(`Equipment Total: Rs.${equipmentTotal.toFixed(2)}`, margin, yPos);
      yPos += 6;
    }
    if (glasswareTotal > 0) {
      doc.text(`Glassware Total: Rs.${glasswareTotal.toFixed(2)}`, margin, yPos);
      yPos += 6;
    }
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(14);
    doc.text(`GRAND TOTAL: Rs.${grandTotal.toFixed(2)}`, margin, yPos);
                  ...
                  <div className="space-y-2 text-sm">
                    {chemicalTotal > 0 && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">Rs.{chemicalTotal.toFixed(2)}</span>
                      </div>
                    )}
                    {equipmentTotal > 0 && (
                      <div className="flex justify-between">
                        <span>Equipment ({quotation.equipment?.length || 0} items):</span>
                        <span className="font-medium">Rs.{equipmentTotal.toFixed(2)}</span>
                      </div>
                    )}
                    {glasswareTotal > 0 && (
                      <div className="flex justify-between">
                        <span>Glassware ({quotation.glassware?.length || 0} items):</span>
                        <span className="font-medium">Rs.{glasswareTotal.toFixed(2)}</span>
                      </div>
                    )}
                    <hr className="my-2" />
                    ...
                    <tbody>
                      {quotation.chemicals.map((chem, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-3">Rs.{chem.pricePerUnit || 0}</td>
                          <td className="p-3 font-medium">Rs.{((chem.pricePerUnit || 0) * chem.quantity).toFixed(2)}</td>
                          <td className="p-3">
                            {editMode ? (
                              <input
                                type="text"
                    ...
                    <tbody>
                      {quotation.equipment.map((eq, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-3">Rs.{eq.pricePerUnit || 0}</td>
                          <td className="p-3 font-medium">Rs.{((eq.pricePerUnit || 0) * eq.quantity).toFixed(2)}</td>
                    ...
                    <tbody>
                      {quotation.glassware.map((glass, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-3">Rs.{glass.pricePerUnit || 0}</td>
                          <td className="p-3 font-medium">Rs.{((glass.pricePerUnit || 0) * glass.quantity).toFixed(2)}</td>
